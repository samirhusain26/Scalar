...
└── src/                             # React/TypeScript source
    ...
    ├── store/
    │   └── gameStore.ts             # Zustand store: single source of truth, localStorage persistence (version 11)
    ├── utils/
...
- **Lucide React** for icons (Eye, Check, ChevronDown)
...
State:
  ...
  columnVisibility: Record<string, boolean>  (all non-folded columns visible by default)
...
Actions:
  ...
  submitGuess(entity) -> computes feedback, increments moves +1, checks win
  revealColumn(attr) -> makes hidden column visible; uses credit if available (0 moves), else +3 moves
  revealFoldedAttribute(attr) -> same credit/moves logic as revealColumn
  ...
  revealAnswer() -> forfeit: set status to REVEALED, moves set to Infinity
...
- **Forfeit**: Reveal Answer ends game as REVEALED, moves set to **∞** (Infinity)

## Column Visibility & Hint Systems
- All non-folded display columns are visible by default on every guess card
- `columnVisibility` state exists in the store — revealing a hidden column uses the credit system (free with credits, +3 moves without)
...
- `revealFoldedAttribute` store action uses same credit/moves logic as `revealColumn`
...
- Store version is 11 - bumping it triggers migration (clears stale localStorage)
...
- `columnVisibility` exists in store — revealing hidden columns uses credit system (free with credits, +3 moves without)
- Folded attributes use `revealFoldedAttribute` store action with same credit/moves logic
...
- **Column visibility**: All non-folded columns visible by default. Hidden columns can be revealed (uses credit system).
...
- **Forfeit**: Reveal Answer shows target + all attributes. Status becomes REVEALED. Moves set to Infinity.
