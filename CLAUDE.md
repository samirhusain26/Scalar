...
└── src/                             # React/TypeScript source
    ...
    ├── assets/
    │   └── data/
    │       ├── gameData.json        # Static game data: schema definitions + entity data per category (auto-generated by fetch_data.py)
    │       └── countries-110m.json  # World TopoJSON (110m resolution, copied from world-atlas npm package)
    ├── store/
    ...
    │   └── countryCodeMap.ts        # M49_TO_ISO3: Record<string, string> — maps UN M49 numeric IDs (world-atlas) to ISO Alpha-3 (entity IDs)
    └── components/
        ...
        ├── WorldMapView.tsx         # SVG world map (d3-geo + world-atlas TopoJSON); highlights guessed/target countries
        ├── PeriodicTableView.tsx    # Full 18×7 CSS Grid periodic table; highlights guessed/target elements
        └── VisualizationModal.tsx   # Radix Dialog wrapping WorldMapView/PeriodicTableView; includes ad placeholder gate
```
...
- **d3-geo** + **topojson-client** for world map SVG rendering in VisualizationModal (world TopoJSON bundled at `src/assets/data/countries-110m.json`)
- **world-atlas** for TopoJSON world country data (110m resolution)
...
### WorldMapView.tsx
- SVG world map rendered with `d3-geo` (`geoNaturalEarth1` projection) + `topojson-client`
- World TopoJSON loaded from `src/assets/data/countries-110m.json` (bundled, no CDN)
- Country identification: M49 numeric geometry IDs → ISO Alpha-3 via `M49_TO_ISO3` lookup
- **Color logic**: guessed (not target) → charcoal; target (SOLVED/REVEALED) → thermal-green; unguessed → paper-white
- SVG background: light blue (`#EFF6FF`) to represent ocean
- Responsive: `ResizeObserver` watches container width, updates projection scale + translate and recomputes all paths
- Tooltip: `<title>` tag on each `<path>` shows country name on hover

### PeriodicTableView.tsx
- Full periodic table built from `gameData.categories.elements` entity data — no external library
- CSS Grid: 18 columns × 10 rows (rows 1-7 main table, row 8 separator label, rows 9-10 lanthanide/actinide series)
- Element positioning: `atomicNum >= 57 && <= 71` → row 9 cols 3-17 (lanthanides); `atomicNum >= 89 && <= 103` → row 10 cols 3-17 (actinides); others → `{ row: period, col: group }`
- Placeholder dashed cells at main table (row 6, col 3) and (row 7, col 3) show "57–71" / "89–103"
- **Color logic**: same as WorldMapView (charcoal=guessed, green=target, family-tinted=unguessed)
- Unguessed cell background tint from `FAMILY_BG` map (10 element families → pastel CSS colors)
- Cell size: 44px; horizontal scroll container for mobile

### VisualizationModal.tsx
- Radix Dialog: `w-[95vw] max-w-4xl`, `max-h-[90vh] flex flex-col`, always centered
- **Ad placeholder gate**: `adWatched: boolean` state — resets to `false` whenever modal closes (each open requires "watching"). Shows: dashed gray `aspect-video` box with `Tv` icon + placeholder text, "Watch Ad to Unlock" button. On click: `setAdWatched(true)` (real ad SDK callback goes here).
- **Visualization**: after `adWatched`, renders `<WorldMapView>` (countries) or `<PeriodicTableView>` (elements) based on `activeCategory`
- Props: `isOpen`, `onClose`, `activeCategory`, `guesses`, `targetEntity`, `gameStatus`
- Opened from App.tsx header button (Map icon for countries, Grid3X3 icon for elements)
